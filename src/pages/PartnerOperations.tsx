...
export default function PartnerOperations({section}:{section:PartnerOpsSection}){
 ...
 useEffect(()=>{if(section==='pipeline'&&!loading){const jobId=params.get('job');if(jobId&&jobs.some(j=>j.id===jobId)){setDiscoveryJob(jobId);setNewPipeline(v=>({...v,job_id:jobId}));setParams({}, {replace:true})}}},[section,loading,jobs,params,setParams]);
 ...
 if(section==='vacancies')return <div className="page partner-page">
  ...
  <Card><div className="card-head"><div><h3>Assigned vacancies</h3><p>{jobs.length} vacancy record{jobs.length===1?'':'s'} currently visible in your portfolio. Draft means approved for internal delivery but not yet public; Published means the public application/referral route is live.</p></div><div className="search"><Search size={14}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search vacancies"/></div></div>
   {!filteredJobs.length?<p className="muted">No assigned vacancies yet.</p>:<div className="table-wrap"><table><thead><tr><th>Vacancy</th><th>Client</th><th>Location</th><th>Status</th><th>Salary</th></tr></thead><tbody>{filteredJobs.map(j=><tr key={j.id}><td><strong>{j.title}</strong><span>{j.employment_type}</span></td><td>{clientsById.get(j.client_id)?.company_name||'Assigned client'}</td><td>{j.location||'—'}</td><td><Badge tone={j.status==='published'?'green':'neutral'}>{j.status==='draft'?'Approved · internal':j.status==='published'?'Published · public':String(j.status).replaceAll('_',' ')}</Badge></td><td>{j.salary_min||j.salary_max?`${j.salary_min?money(j.salary_min):'—'} – ${j.salary_max?money(j.salary_max):'—'}`:'Not stated'}<div className="button-row"><Button variant="ghost" onClick={()=>setSelectedJob(j)}>View brief</Button>{access.partnerCanSourceCandidates&&<Button variant="ghost" onClick={()=>location.assign('/dashboard/partner/pipeline?job='+encodeURIComponent(j.id))}>Source candidates</Button>}{access.partnerCanSourceCandidates&&j.status==='published'&&<Button variant="ghost" disabled={busy} onClick={()=>createReferralLink(j)}>Copy referral link</Button>}</div></td></tr>)}</tbody></table></div>}
  </Card>
  {selectedJob&&<Card><div className="card-head"><div><div className="eyebrow">{selectedJob.status==='published'?'PUBLISHED VACANCY':'APPROVED INTERNAL VACANCY'}</div><h3>{selectedJob.title}</h3><p>{clientsById.get(selectedJob.client_id)?.company_name||'Client'} · {selectedJob.location}</p><p className="muted">{selectedJob.status==='published'?'Public applications/referral links are enabled.':'You can source against this approved vacancy internally, but public applications/referral links remain disabled until Vorlen publishes it.'}</p>{access.partnerCanSourceCandidates&&<div className="button-row"><Button onClick={()=>location.assign('/dashboard/partner/pipeline?job='+encodeURIComponent(selectedJob.id))}>Start sourcing</Button></div>}</div><button className="close" onClick={()=>setSelectedJob(null)}>×</button></div><div className="grid two"><div><strong>Description</strong><p className="prose">{selectedJob.description||'No description recorded.'}</p></div><div><strong>Requirements</strong><p className="prose">{Array.isArray(selectedJob.requirements)?selectedJob.requirements.join('\n'):selectedJob.requirements||'No requirements recorded.'}</p></div><div><strong>Duties</strong><p className="prose">{selectedJob.duties||'Not recorded.'}</p></div><div><strong>Qualifications</strong><p className="prose">{selectedJob.required_qualifications||'Not recorded.'}</p></div><div><strong>Working pattern</strong><p>{selectedJob.work_days_hours||'Not recorded.'}</p></div><div><strong>Start / duration</strong><p>{selectedJob.start_date||'Not specified'}{selectedJob.duration_text?' · '+selectedJob.duration_text:''}</p></div></div></Card>}
 </div>;
 ...
 if(section==='handoffs')return <div className="page partner-page">
  ...
  <Card><h3>My handoffs</h3>{handoffs.map(h=><div className="list-row partner-handoff-row" key={h.id}><div><strong>{h.vacancy_title}</strong><span>{h.client_id?clientsById.get(h.client_id)?.company_name:(h.prospect_id?prospects.find(p=>p.id===h.prospect_id)?.company_name:h.prospect_company)} · {h.vacancy_location||'Location not stated'}</span><span><b>Next:</b> {h.status==='draft'?'Finish the requirement and submit it to Vorlen.':h.status==='submitted'||h.status==='under_review'?'Vorlen management is reviewing the commercial request; no binding terms should be agreed meanwhile.':h.status==='terms_approved'?'Approved terms are ready; progress the client toward acceptance / vacancy conversion.':h.status==='converted'?'Vacancy created and assigned — continue delivery from Vacancies.':h.status==='declined'?'Review the manager note before taking any further action.':'Check the current status with Vorlen.'}</span>{h.manager_notes&&<span>Vorlen: {h.manager_notes}</span>}{h.approved_job_id&&<span>Vacancy linked: {jobsById.get(h.approved_job_id)?.title||h.approved_job_id}</span>}{h.updated_at&&<small>Updated {new Date(h.updated_at).toLocaleString('en-GB')}</small>}</div><div className="button-row"><Badge tone={handoffTone(h.status) as any}>{handoffLabel(h.status)}</Badge>{h.status==='draft'&&<Button variant="ghost" onClick={()=>startHandoff(h)}>Edit</Button>}{h.status==='converted'&&<Button variant="ghost" onClick={()=>location.assign('/dashboard/partner/vacancies')}>Open vacancy</Button>}</div></div>)}{!handoffs.length&&<p className="muted">No handoffs yet.</p>}</Card>
 </div>;
 ...
 return <div className="page partner-page">
  ...
  <div className="grid two">
   ...
   <Card><LifeBuoy size={20}/><h3>When to escalate</h3><p>Escalate contractual questions, complaints, data-rights requests, unusual candidate safeguarding issues, fee negotiations and anything that could legally or financially bind Vorlen.</p><div className="button-row"><a className="btn ghost" href="/contact">Contact Vorlen</a><a className="btn ghost" href="/dashboard/partner?view=tasks">Open my tasks</a></div></Card>
  </div>
 </div>
}
