...
export default function PartnerOperations({section}:{section:PartnerOpsSection}){
 ...
 if(section==='vacancies')return <div className="page partner-page">
  <div className="page-actions"><div><div className="eyebrow">MY VACANCIES</div><h2>Vacancy workspace</h2><p>Work only the genuine vacancies assigned to you by Vorlen.{access.partnerCanDevelopClients?' New opportunities start as a commercial handoff.':''}</p></div>{access.partnerCanDevelopClients&&<Button onClick={()=>location.assign('/dashboard/partner/handoffs?new=1')}><Plus size={15}/> Submit hiring opportunity</Button>}</div>
  {error&&<div className="notice error">{error}</div>}
  <div className="notice"><strong>Vacancy control:</strong> These are manager-authorised Vorlen vacancy records assigned to you. {access.partnerCanDevelopClients?'A new employer opportunity is not live until commercial terms are approved and a manager creates or links the authorised job record.':'Your role can work assigned vacancy briefs but does not create employer prospects or commercial handoffs.'}</div>
 ...
 if(section==='handoffs'&&!access.partnerCanDevelopClients)return <div className="page"><Card><h2>Commercial handoffs not enabled</h2><p>Your current partner specialism does not include employer development or commercial handoffs.</p></Card></div>;
 ...
 return <div className="page partner-page">
  ...
  <div className="grid two">
   <Card><Handshake size={20}/><h3>Client development</h3><p>{access.partnerCanDevelopClients?'Identify genuine UK employers and hiring needs, record every material interaction in Vorlen, and move interested employers into a commercial handoff. Never represent an unapproved opportunity as a live Vorlen vacancy.':'This capability is not assigned to your current partner specialism. Employer prospecting and commercial handoffs should not be carried out from your account.'}</p></Card>
