...
export default function Overview(){
  ...
  if(error)return <div className="page"><div className="alert error" role="alert">{error}</div><p className="muted">Vorlen could not load your workspace. Retry the page; if the problem persists, ask a workspace owner to check Settings.</p></div>;
  ...
  const canManage=access.canManageWorkspace;
  const setup=canManage?[{done:(data.clients||[]).length>0,label:'Add your first client',to:'/dashboard/clients'},{done:stats.jobs.length>0,label:'Create a job',to:'/dashboard/jobs'},{done:stats.published>0,label:'Publish a role',to:'/dashboard/jobs'},{done:stats.candidates.length>0,label:'Receive or add a candidate',to:'/dashboard/candidates'}]:[{done:stats.jobs.length>0,label:'Review assigned jobs',to:'/dashboard/jobs'},{done:stats.candidates.length>0,label:'Review candidate pipeline',to:'/dashboard/candidates'},{done:stats.apps.length>0,label:'Review applications',to:'/dashboard/applications'}];
  const setupDone=setup.filter(x=>x.done).length;

  return <div className="page overview-page">
    <div className="page-actions"><div><div className="eyebrow">WORKSPACE</div><h2>Overview</h2><p>What needs attention across your recruitment operation.</p></div><div className="button-row"><Link className="btn ghost" to="/dashboard/jobs">{canManage?'Create job':'View jobs'}</Link><Link className="btn" to="/dashboard/candidates">{canManage?'Add candidate':'Open candidates'}</Link></div></div>
    ...
    <div className="grid two"><Card><div className="card-head"><div><h2>Live jobs</h2><p>Roles currently accepting candidates.</p></div><Link className="text-link" to="/dashboard/jobs">View all jobs</Link></div>{stats.jobs.filter((j:any)=>j.status==='published').slice(0,5).map((j:any)=><div className="list-row" key={j.id}><div><strong>{j.title}</strong><span>{j.location||'Location not set'} · {j.employment_type||'Type not set'}</span></div><Badge tone="green">Published</Badge></div>)}{stats.published===0&&<div className="empty small"><p>No published jobs yet.</p><Link className="text-link" to="/dashboard/jobs">{canManage?'Create or publish a role':'View jobs'}</Link></div>}</Card><Card><div className="card-head"><div><h2>Recent candidates</h2><p>Latest people entering your funnel.</p></div><Link className="text-link" to="/dashboard/candidates">View all candidates</Link></div>{stats.candidates.slice(0,6).map((c:any)=><Link className="list-row overview-row-link" to={`/dashboard/candidates/${c.id}`} key={c.id}><div><strong>{c.full_name||'Unnamed candidate'}</strong><span>{c.email||'No email'} · {c.source||'Direct'}</span></div><Badge tone={c.stage==='placed'?'green':c.stage==='rejected'?'red':'blue'}>{c.stage||'new'}</Badge></Link>)}{stats.candidates.length===0&&<div className="empty small"><p>No candidates yet.</p><Link className="text-link" to="/dashboard/candidates">Open candidates</Link></div>}</Card></div>
    <Card><div className="card-head"><div><h2>Recruitment flow</h2><p>Vorlen keeps the hand-offs visible while AI remains decision support, not the hiring decision-maker.</p></div></div><div className="funnel"><span>Traffic</span><i>→</i><span>Application</span><i>→</i><span>AI evidence</span><i>→</i><span>Recruiter review</span><i>→</i><span>Client interview</span><i>→</i><span>Placement</span></div></Card>
  </div>
}
