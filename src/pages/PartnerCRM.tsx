...
export default function PartnerCRM(){
 ...
 const contacts=snap?.contacts||[],timeline=snap?.timeline||[],opps=snap?.opportunities||[],sequences=snap?.sequences||[],templates=snap?.templates||[],vacancyRequests=snap?.vacancy_requests||[],portalActivity=snap?.portal_activity||{};
 ...
 return <div className="page partner-page">
  ...
  {!selected?<Card><p className="muted">No assigned client account is available.</p></Card>:busy==='load'&&!snap?<SkeletonRows rows={4}/>:<>
   ...
   {tab==='opportunities'&&<><Card><div className="card-head"><div><h3>Client portal activity</h3><p>Requests and decisions coming back from the client portal are visible here without giving partners client impersonation access.</p></div></div><div className="partner-kpi-grid"><div><span>Vacancy requests</span><strong>{vacancyRequests.length}</strong><small>{vacancyRequests.filter((v:any)=>!['converted','declined'].includes(v.status)).length} open</small></div><div><span>Candidate submissions</span><strong>{portalActivity.submissions||0}</strong><small>{portalActivity.awaiting_client_decision||0} awaiting client</small></div><div><span>Scheduled interviews</span><strong>{portalActivity.interviews||0}</strong><small>for this account</small></div></div>{vacancyRequests.slice(0,8).map((v:any)=><div className="list-row" key={v.id}><div><strong>{v.title}</strong><span>{String(v.status).replaceAll('_',' ')}{v.location?' · '+v.location:''}</span><small>{v.hiring_need}</small>{v.manager_notes&&<small>Vorlen: {v.manager_notes}</small>}</div><Badge tone={v.status==='converted'?'green':v.status==='declined'?'red':'amber'}>{String(v.status).replaceAll('_',' ')}</Badge></div>)}</Card><div className="grid two"><Card><div className="card-head"><div><h3><Target size={16}/> Opportunity pipeline</h3><p>A single sales pipeline from identified need through accepted terms, vacancy and win/loss.</p></div></div>{opps.map((o:any)=><div className="list-row" key={o.id}><div><strong>{o.name}</strong><span>{o.stage.replaceAll('_',' ')} · {o.probability}%{o.expected_fee?' · '+money(o.expected_fee):''}</span>{o.next_action&&<span>Next: {o.next_action}{o.next_action_at?' · '+fmt(o.next_action_at):''}</span>}</div><Button variant="ghost" onClick={()=>moveOpportunity(o)}>Update</Button></div>)}{!opps.length&&<p className="muted">No opportunities yet.</p>}</Card>
