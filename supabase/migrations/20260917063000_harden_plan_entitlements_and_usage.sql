...
begin
  ...
  select jsonb_build_object('company_id',c,'plan_code',s.plan_code,'plan_name',p.name,'status',s.status,'trial_ends_at',s.trial_ends_at,'current_period_end',s.current_period_end,'cancel_at_period_end',s.cancel_at_period_end,'features',p.features,'limits',jsonb_build_object('recruiters',p.recruiter_limit,'active_jobs',p.active_job_limit,'candidates',p.candidate_limit,'automations',p.automation_limit),'usage',jsonb_build_object('recruiters',(select count(*) from public.profiles x where x.company_id=c and x.role in ('owner','manager','recruiter')),'active_jobs',(select count(*) from public.jobs x where x.company_id=c and x.status::text='published'),'candidates',(select count(*) from public.candidates x where x.company_id=c),'automations',(select count(*) from public.automation_sequences x where x.company_id=c))) into result from public.company_subscriptions s join public.saas_plans p on p.code=s.plan_code where s.company_id=c;
