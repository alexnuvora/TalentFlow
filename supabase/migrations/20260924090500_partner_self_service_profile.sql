...
begin
  ...
  set display_name=nullif(btrim(p_display_name),''),linkedin_url=nullif(btrim(p_linkedin_url),''),
      timezone=coalesce(nullif(btrim(p_timezone),''),'UTC'),sectors=coalesce(p_sectors,'{}'::text[]),
      regions=coalesce(p_regions,'{}'::text[]),role_types=coalesce(p_role_types,'{}'::text[]),
      availability_hours=nullif(btrim(p_availability_hours),''),profile_photo_url=nullif(btrim(p_profile_photo_url),''),
      updated_at=now()
  ...
  set legal_name=nullif(btrim(p_legal_name),''),trading_name=nullif(btrim(p_trading_name),''),
      country=nullif(btrim(p_country),''),address=nullif(btrim(p_address),''),phone=nullif(btrim(p_phone),''),
      business_type=nullif(btrim(p_business_type),''),company_registration_number=nullif(btrim(p_company_registration_number),''),
      vat_number=nullif(btrim(p_vat_number),''),tax_reference=nullif(btrim(p_tax_reference),''),
      payment_method=nullif(btrim(p_payment_method),''),payment_account_name=nullif(btrim(p_payment_account_name),''),
