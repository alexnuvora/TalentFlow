...
begin
  ...
  if v_privileged then
    if new.status in ('terms_approved','converted') then
      ...
      if not exists (
        ...
        where cc.company_id=new.company_id and cc.client_id=new.client_id and cc.status='active'
      ) then raise exception 'An active Vorlen client contract is required before commercial terms can be approved'; end if;
    ...
    if new.status='converted' then
      ...
      if not exists (
        ...
        where j.id=new.approved_job_id and j.company_id=new.company_id and j.client_id=new.client_id
      ) then raise exception 'The linked vacancy must belong to the approved client'; end if;
    end if;
    return new;
  end if;
  ...
  if tg_op='INSERT' then
    ...
    new.manager_notes := old.manager_notes; new.approved_by := old.approved_by;
    new.approved_at := old.approved_at; new.approved_job_id := old.approved_job_id;
