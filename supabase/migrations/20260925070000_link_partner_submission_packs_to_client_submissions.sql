...
begin
  ...
  set candidate_submission_id=new.id,
      status=case
        when new.status not in ('draft','withdrawn','approved_to_send') then 'submitted'
        else p.status
      end,
      updated_at=now()
...
drop trigger if exists trg_sync_partner_submission_pack_to_client_submission
on public.candidate_submissions;

create trigger trg_sync_partner_submission_pack_to_client_submission
after insert or update of status
on public.candidate_submissions
for each row
execute function public.sync_partner_submission_pack_to_client_submission();

revoke all on function public.sync_partner_submission_pack_to_client_submission()
from public,anon,authenticated;

-- Backfill any already-existing official submissions against approved partner packs.
update public.partner_submission_packs p
set candidate_submission_id=s.id,
    status=case when s.status not in ('draft','withdrawn','approved_to_send') then 'submitted' else p.status end,
    updated_at=now()
