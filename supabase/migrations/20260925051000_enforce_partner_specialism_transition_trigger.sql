...
-- Enforce specialism transitions at the table boundary as well as through the RPC.

create or replace function private.enforce_partner_specialism_transition()
returns trigger
language plpgsql
security definer
set search_path=''
...
begin
  if new.specialism is not distinct from old.specialism then
    return new;
  end if;

  if new.specialism not in ('b2b_advisor','lead_closer','candidate_sourcer','hybrid') then
    raise exception 'Invalid partner specialism';
  end if;

  select o.status into v_onboarding_status
  from public.partner_onboarding o
  where o.partner_id=new.user_id and o.company_id=new.company_id;

  if v_onboarding_status='terminated' then
    raise exception 'Terminated partners cannot be reconfigured';
  end if;

  if new.specialism not in ('b2b_advisor','lead_closer','hybrid') and exists(
    select 1 from public.partner_assignments a
    where a.company_id=new.company_id and a.partner_id=new.user_id
  ...
  if new.specialism not in ('candidate_sourcer','hybrid') and exists(
    select 1 from public.partner_assignments a
    where a.company_id=new.company_id and a.partner_id=new.user_id
  ...
  if new.specialism not in ('lead_closer','candidate_sourcer','hybrid') and exists(
    select 1 from public.partner_assignments a
    where a.company_id=new.company_id and a.partner_id=new.user_id
      and a.job_id is not null and a.completed_at is null
  ) then
    raise exception 'Complete or reassign active vacancy assignments before removing vacancy-work permission';
  end if;

  if new.specialism not in ('b2b_advisor','hybrid') then
    if exists(
      select 1 from public.partner_prospects p
      where p.company_id=new.company_id and p.partner_id=new.user_id
    ...
    if exists(
      ...
      where h.company_id=new.company_id and h.from_partner=new.user_id and h.status='requested'
    ) then
      raise exception 'Resolve pending advisor-to-closer handovers before removing B2B Advisor permission';
    end if;
  end if;

  if new.specialism not in ('lead_closer','hybrid') then
    if exists(
      select 1 from public.partner_commercial_handoffs h
      where h.company_id=new.company_id and h.partner_id=new.user_id
  ...
  if new.specialism not in ('candidate_sourcer','hybrid') and exists(
    ...
    where r.company_id=new.company_id and r.partner_id=new.user_id and r.status='pending'
  ) then
...
drop trigger if exists trg_partner_specialism_transition on public.partner_profiles;
create trigger trg_partner_specialism_transition
before update of specialism on public.partner_profiles
for each row execute function private.enforce_partner_specialism_transition();
...
begin
  ...
  -- The partner_profiles trigger is the final integrity boundary for active work.
