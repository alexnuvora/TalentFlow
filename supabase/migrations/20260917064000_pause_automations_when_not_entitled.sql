...
begin
  select coalesce((p.features->>'automations')::boolean,false) and new.status in ('active','trialing') and (new.status<>'trialing' or new.trial_ends_at>now()) into allowed from public.saas_plans p where p.code=new.plan_code and p.active=true;
  if coalesce(allowed,false)=false then update public.automation_sequences set active=false where company_id=new.company_id and active=true; end if;
  return new;
end $$;
