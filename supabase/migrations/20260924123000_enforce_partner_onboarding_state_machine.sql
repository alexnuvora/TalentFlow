...
begin
 ...
 if auth.uid() is null or old.partner_id<>auth.uid() or old.company_id<>private.current_company_id() then raise exception 'You may only update your own partner onboarding record';end if;
 ...
 if old.status='terms_pending' then raise exception 'Accept the partner agreement before completing onboarding details';
 elsif old.status='details_pending' then if new.status not in ('details_pending','review_pending') then raise exception 'Partner details can only remain in progress or be submitted for review';end if;
 elsif old.status='review_pending' then if new.status<>'review_pending' then raise exception 'Submitted onboarding remains under review until Vorlen management acts';end if;
...
drop trigger if exists trg_partner_onboarding_boundary on public.partner_onboarding;
create trigger trg_partner_onboarding_boundary before update on public.partner_onboarding for each row execute function private.enforce_partner_onboarding_boundary();
revoke all on function private.enforce_partner_onboarding_boundary() from public,anon,authenticated;
grant execute on function private.enforce_partner_onboarding_boundary() to service_role;
