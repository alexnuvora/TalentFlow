...
Deno.serve(async req=>{
  ...
  try{
    ...
    const{data:s}=await db.from('candidate_submissions').select('id,company_id,client_id,job_id,candidate_id,application_id,recruiter_summary,status,client_feedback,client_decision,submitted_at,candidate_authorisation,client_safe_cv_text,client_safe_cv_source_hash').eq('id',t.submission_id).eq('company_id',t.company_id).single();
