...
async function safeCv(db:any,s:any,c:any){if(!c.resume_path)return null;const{data:file,error}=await db.storage.from('candidate-resumes').download(c.resume_path);if(error||!file)return null;const ex=await extractCv(file,c.resume_path);const text=redact(ex.text,c);if(text.length<80)return null;return text}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    const{data:s}=await db.from('candidate_submissions').select('id,company_id,client_id,job_id,candidate_id,application_id,recruiter_summary,status,client_feedback,client_decision,submitted_at,candidate_authorisation').eq('id',t.submission_id).eq('company_id',t.company_id).single();
