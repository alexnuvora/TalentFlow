...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(name==='search_candidates'){const q=clean(a.query,120);let x=db.from('candidates').select('id,full_name,email,phone,location,stage,score,source,recruiter_summary,next_action,next_action_at,created_at').eq('company_id',company_id).is('erased_at',null).limit(25);if(q)x=x.or(`full_name.ilike.%${safe(q)}%,email.ilike.%${safe(q)}%`);const {data,error}=await x;if(error)throw error;return done(data||[])}
